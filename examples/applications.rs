...
fn main() {
    ...
    let velocity = diff(position, "t", None, None).unwrap();
    let acceleration = diff(&velocity, "t", None, None).unwrap();
    ...
    let current = diff(voltage, "t", Some(&["V0", "R", "C"]), None).unwrap();
    ...
    let heat_flux = diff(temperature, "x", Some(&["T0", "alpha", "t"]), None).unwrap();
    ...
    let prob_derivative = diff(&probability_density, "x", Some(&["sigma"]), None).unwrap();
    ...
    let shear_stress = diff(velocity, "r", Some(&["deltaP", "mu", "L", "R"]), None).unwrap();
    ...
    let mag_sensitivity = diff(magnification, "do", Some(&["di"]), None).unwrap();
    ...
    let denom_derivative = diff(denominator, "s", Some(&["zeta", "omega_n"]), None).unwrap();
    ...
    let pdf_derivative = diff(normal_pdf, "x", Some(&["mu", "sigma"]), None).unwrap();
    ...
    let reaction_rate = diff(concentration, "t", Some(&["A0", "k"]), None).unwrap();

    // Negate the derivative to get -d[A]/dt (rate of consumption)
    let negated_rate = format!("-1 * ({})", reaction_rate);
    let consumption_rate = symb_anafis::simplify(&negated_rate, Some(&["A0", "k"]), None).unwrap();
    ...
    let particle_velocity = diff(pressure, "x", Some(&["p0", "k", "omega", "t"]), None).unwrap();
    ...
    let rate_equation = diff(constraint, "t", None, None).unwrap();
    ...
    let volume = "x * y * z";
    let volume_grad_x = diff(volume, "x", Some(&["y", "z"]), None).unwrap();
    ...
    // f(x) ≈ f(a) + f'(a)(x-a)
    let function = "sin(x)";
    let f1 = diff(function, "x", None, None).unwrap();
    ...
    let dx_dt = diff(x_parametric, "t", None, None).unwrap();
    let dy_dt = diff(y_parametric, "t", None, None).unwrap();
    ...
    let ds_inner = symb_anafis::simplify(&ds_squared, None, None).unwrap();
    let ds = format!("sqrt({})", ds_inner);
    let ds_simplified = symb_anafis::simplify(&ds, None, None).unwrap();
    ...
    let velocity = diff(displacement, "t", Some(&["A", "omega", "phi"]), None).unwrap();
    let acceleration = diff(&velocity, "t", Some(&["A", "omega", "phi"]), None).unwrap();
    ...
    let emf = diff(magnetic_field, "t", Some(&["B0", "tau"]), None).unwrap();

    // Negate for Faraday's law: EMF = -dB/dt
    let negated_emf = format!("-1 * ({})", emf);
    let induced_emf = symb_anafis::simplify(&negated_emf, Some(&["B0", "tau"]), None).unwrap();
    ...
    let entropy_temp_deriv = diff(entropy, "T", Some(&["Cv", "R", "V"]), None).unwrap();
    ...
    let time_derivative = diff(wave_function, "t", Some(&["psi0", "E", "hbar"]), None).unwrap();
    ...
    let time_dilation_deriv = diff(proper_time, "v", Some(&["t", "c"]), None).unwrap();
    ...
    let angular_momentum = diff(radius, "theta", Some(&["a", "ecc"]), None).unwrap();
