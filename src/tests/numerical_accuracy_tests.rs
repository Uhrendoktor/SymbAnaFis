...
fn test_trig_derivative_accuracy() {
    // d/dx[sin(x)] = cos(x)
    let result = diff("sin(x)", "x", None, None).unwrap();
    assert_eq!(result, "cos(x)");

    // d/dx[cos(x)] = -sin(x)
    let result = diff("cos(x)", "x", None, None).unwrap();
...
fn test_exponential_log_accuracy() {
    // d/dx[e^x] = e^x
    let result = diff("exp(x)", "x", None, None).unwrap();
    assert_eq!(result, "exp(x)");

    // d/dx[ln(x)] = 1/x
    let result = diff("ln(x)", "x", None, None).unwrap();
...
fn test_product_rule_accuracy() {
    ...
    let result = diff("x*sin(x)", "x", None, None).unwrap();
...
fn test_quotient_rule_accuracy() {
    ...
    let result = diff("sin(x)/x", "x", None, None).unwrap();
...
fn test_chain_rule_accuracy() {
    ...
    let result = diff("sin(x^2)", "x", None, None).unwrap();
...
fn test_constant_derivatives() {
    ...
    // d/dx[π] = 0
    let result = diff("pi", "x", None, None).unwrap();
    assert_eq!(result, "0");
}
...
fn test_linear_derivatives() {
    // d/dx[ax + b] = a
    let result = diff("3*x + 5", "x", None, None).unwrap();
    assert_eq!(result, "3");

    // d/dx[ax] = a
    let result = diff("7*x", "x", None, None).unwrap();
    assert_eq!(result, "7");
}
